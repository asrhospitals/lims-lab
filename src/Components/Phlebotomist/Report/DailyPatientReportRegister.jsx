...
const DailyPatientReportRegister = () => {
  ...
  useEffect(() => {
    const data = [
      { id: 1, patientcode: "Mahukaram", patientname: "Cardiology", barcode: "MRN001", dateofregistration: "9876543210", hospitalname: "john@example.com" },
      { id: 2, patientcode: "Smitha", patientname: "Neurology", barcode: "MRN002", dateofregistration: "9876543211", hospitalname: "jane@example.com" },
      { id: 3, patientcode: "Aparna", patientname: "Orthopedics", barcode: "MRN003", dateofregistration: "9876543212", hospitalname: "alice@example.com" },
    ];
  ...
  useEffect(() => {
    const fetchNewReports = async () => {
      try {
        const token = localStorage.getItem("authToken");
        const response = await axios.get(
          `https://asrphleb.asrhospitalindia.in/api/v1/phleb/report/new`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      ...
      <div className="fixed top-[61px] w-full z-10">
        <nav className="flex items-center text-semivold font-medium justify-start px-4 py-2 bg-gray-50 border-b shadow-lg transition-colors">
          <ol className="inline-flex items-center space-x-1 md:space-x-3 text-sm font-medium">
            ...
            <li>
              <Link to="/view-report-doctor" className="text-gray-700 hover:text-teal-600 transition-colors">
                Daily Patient Report Register
              </Link>
            </li>
            <li className="text-gray-400">/</li>
      ...
      <div className="w-full mt-12 px-0 sm:px-2 space-y-4 text-sm">
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-800">Daily Patient Report Register</h2>
            ...
            <div className="flex items-center gap-3">
              ...
              <div ref={notifRef} className="bg-yellow-100 rounded-lg p-2 cursor-pointer hover:bg-yellow-200 transition flex items-center justify-center relative">
                <button onClick={() => setIsNotifOpen(!isNotifOpen)} className="relative text-gray-700">
                  ...
                  <div className="absolute right-0 mt-2 w-80 bg-white border rounded-lg shadow-lg z-50 p-2 max-h-80 overflow-y-auto">
                    ...
                    {notifications.map((notif) => (
                      <div key={notif.id} className="flex justify-between items-center text-sm py-1 px-2 border-b last:border-b-0">
                        ...
                        <button onClick={() => setNotifications((prev) => prev.filter((n) => n.id !== notif.id))} className="px-2 py-1 bg-red-500 text-white rounded-full text-xs hover:bg-red-600">
                          ✕
                        </button>
                      </div>
                    ))}
          ...
          <div className="flex flex-col sm:flex-row items-end gap-4 mb-4 flex-wrap">
            <div className="flex flex-col sm:flex-row gap-2 items-end">
              <div className="flex-1 min-w-[250px]">
                <select {...register("hospitalselected", { required: "Hospital is required" })}
                  className={`w-full px-4 py-2 rounded-lg border ${errors.hospitalselected ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-teal-500"} focus:ring-2 transition`}>
            ...
            <div className="flex flex-col sm:flex-row gap-2 items-end">
              <div className="flex-1 min-w-[160px]">
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition" />
              </div>
              <div>
          ...
          {mappedItems.length === 0 ? (
            ...
            <PhlebotomistDataTable items={mappedItems} columns={columns} itemsPerPage={10} showDetailsButtons={false} onUpdate={handleUpdate} />
          )}
        </div>
      </div>
    </>
...
export default DailyPatientReportRegister;
