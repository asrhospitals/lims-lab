...
const UpdateSpecimenType = () => {
  ...
  useEffect(() => {
    const fetchSpecimen = async () => {
      try {
        ...
        const specimen = response.data.data;

        reset({
          specimenname: specimen.specimenname || "",
          specimendes: specimen.specimendes || "",
          isactive: String(specimen.isactive),
        });
      } catch (err) {
        toast.error("❌ Failed to fetch specimen details.");
        navigate("/view-specimen-types"); // fallback
      }
    };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/view-specimen-type"), 1000);
    } catch (err) {
      toast.error(
        err.response?.data?.message || "❌ Failed to update specimen."
      );
    } finally {
      ...
      <div className="fixed top-[61px] w-full z-10">
        <nav className="flex items-center text-sm font-medium px-4 py-2 bg-gray-50 border-b shadow-md">
          <ol className="inline-flex items-center space-x-2 sm:space-x-3">
            ...
            <li className="text-gray-400">/</li>
            <li className="text-gray-500">Update Specimen Type</li>
          </ol>
        </nav>
      </div>
