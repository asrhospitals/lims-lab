...
const UpdateTechnician = () => {
  ...
  const onSubmit = async (data) => {
    if (!technicianToUpdate?.id) {
      toast.error("❌ Technician ID not found. Cannot update.");
      return;
    }
      ...
      <div className="fixed top-[61px] w-full z-10">
        <nav className="flex items-center text-semivold font-medium justify-start px-4 py-2 bg-gray-50 border-b shadow-lg transition-colors" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-3 text-sm font-medium">
            <li>
              <Link to="/" className="inline-flex items-center text-gray-700 hover:text-teal-600">🏠︎ Home</Link>
            </li>
            <li className="text-gray-400">/</li>
            <li>
              <Link to="/view-technician" className="text-gray-700 hover:text-teal-600">Technician</Link>
            </li>
            <li className="text-gray-400">/</li>
            <li className="text-gray-500" aria-current="page">Update Technician</li>
          </ol>
        </nav>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200">
                      ...
                      {options.map((option) => (
                        <label key={option.value} className="inline-flex items-center">
                          <input
                            ...
                            value={option.value}
                            checked={technicianToUpdate[name] === option.value}
          ...
          <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-4">
            ...
            <button
              ...
              className="px-6 py-2 rounded bg-teal-600 hover:bg-teal-700 text-white font-semibold"
            >
