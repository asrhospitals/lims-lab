...
const ViewReportDoctor = () => {
  ...
  useEffect(() => {
    const fetchReportDoctors = async () => {
      try {
        const authToken = localStorage.getItem("authToken");
        const response = await axios.get(
          "https://asrlab-production.up.railway.app/lims/master/get-reportdoctor",
          {
            headers: {
        ...
        setError(err.response?.data?.message || "Failed to fetch Report Doctors.");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (!search.trim()) {
      ...
      const filtered = reportDoctors.filter((doc) =>
        ...
        (doc.phoneNo || "").toLowerCase().includes(lower)
      );
  ...
  const handleUpdate = (doctor) => {
    setReportDoctorToUpdate(doctor);  
    localStorage.setItem("reportDoctorToUpdate", JSON.stringify(doctor));  
    navigate("/update-report-doctor");  
  };
  ...
  const columns = [
    ...
    { key: "medicalRegNo", label: "Medical Reg No" },
      ...
      <div className="fixed top-[61px] w-full z-10">
        <nav className="flex items-center text-semivold font-medium justify-start px-4 py-2 bg-gray-50 border-b shadow-lg transition-colors" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-3 text-sm font-medium">
            ...
            <li>
              <Link to="/view-report-doctor" className="text-gray-700 hover:text-teal-600 transition-colors">Report Doctor</Link>
            </li>
            <li className="text-gray-400">/</li>
      ...
      <div className="w-full mt-12 px-0 sm:px-2 space-y-4 text-sm">
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
            <h2 className="text-lg sm:text-xl font-bold text-gray-800">Report Doctor List</h2>
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:w-64">
                <input
                  ...
                  placeholder="Search report doctor..."
                />
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <button
              onClick={() => navigate("/add-report-doctor")}  
          ...
          {loading ? (
            ...
            <div className="text-center py-6 text-gray-500">No report doctors found.</div>
          ) : (
            <DataTable
              items={mappedItems}
              columns={columns}
