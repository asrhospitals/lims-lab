...
const fields = [
  ...
  },
  {
    ...
    onBlur: (e, errors) => {
      if (errors?.kitname) {
        const input = document.querySelector(`[name="kitname"]`);
        if (input) input.focus();
      }
    },
  },
  { name: "negetiveindex", label: "Negative Index", placeholder: "e.g. <=1" },
   { name: "boderlineindex", label: "Borderline Index", placeholder: "e.g. =>2", }, 
  { name: "positiveindex", label: "Positive Index", placeholder: "e.g. 1" },
  {
    ...
    onBlur: (e, errors) => {
      if (errors?.method) {
        const input = document.querySelector(`[name="method"]`);
        if (input) input.focus();
      }
    },
  },
  {
    ...
    validation: {
      ...
      pattern: { value: /^[0-9]+$/, message: "Batch No must be an integer" },
    },
    onBlur: (e, errors) => {
      if (errors?.batchno) {
        const input = document.querySelector(`[name="batchno"]`);
        if (input) input.focus();
      }
    },
  },
  {
    ...
    validation: {
      ...
      pattern: { value: /^[0-9]+$/, message: "Units must be an integer" },
    },
    onBlur: (e, errors) => {
      if (errors?.units) {
        const input = document.querySelector(`[name="units"]`);
        if (input) input.focus();
      }
    },
  },
  // { name: "negetiveinterpret", label: "Negative Interpretation", placeholder: "e.g. -09" },
  // { name: "borderlineinterpret", label: "Borderline Interpretation", placeholder: "e.g. +09" },
  // { name: "positiveinterpret", label: "Positive Interpretation", placeholder: "e.g. 85" },
];
