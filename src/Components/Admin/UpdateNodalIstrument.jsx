...
import { addNodalInstrument, getAllNodals, viewAllInstrument } from "../../services/apiService";
...
const AddNodalInstrument = () => {
  ...
  useEffect(() => {
    const fetchDropdowns = async () => {
      try {
        const nodals = await getAllNodals();
        const instruments = await viewAllInstrument();
        setNodalList(nodals || []);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      reset({ nodalid: "", instrumentid: "", isactive: "true" });
      setTimeout(() => navigate("/view-nodal-instruments"), 1500);
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "❌ Failed to add Nodal Instrument."
      );
    } finally {
      ...
      <div className="fixed top-[61px] w-full z-10">
        <nav className="flex items-center font-medium justify-start px-4 py-2 bg-gray-50 border-b shadow-lg" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-3 text-sm font-medium">
            ...
            <li><Link to="/view-nodal-instruments" className="text-gray-700 hover:text-teal-600">Nodal Instruments</Link></li>
            <li className="text-gray-400">/</li>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1">
              <label className="block text-sm font-medium text-gray-700">Nodal Name <span className="text-red-500">*</span></label>
              <select
                {...register("nodalid", { required: "Nodal is required" })}
                defaultValue={watch("nodalid")}
            ...
            {/* Instrument dropdown */}
            <div className="space-y-1">
              <label className="block text-sm font-medium text-gray-700">Instrument Name <span className="text-red-500">*</span></label>
              <select
                {...register("instrumentid", { required: "Instrument is required" })}
                defaultValue={watch("instrumentid")}
                ...
                {["true", "false"].map(val => (
                  <label key={val} className="inline-flex items-center">
                    <input
                      ...
                      value={val}
                      defaultChecked={val === "true"} // ✅ default Yes
          ...
          <div className="mt-8 px-6 py-4 flex justify-end gap-4">
            <button type="button" onClick={() => reset({ nodalid: "", instrumentid: "", isactive: "true" })} className="px-6 py-2 border rounded-lg text-gray-700 hover:bg-gray-50">Reset</button>
