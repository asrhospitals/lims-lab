...
const AddInvestigation = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [deptRes, subDeptRes, roleRes, hospRes, specRes, nodalRes, instrRes] = await Promise.all([
          ...
          axios.get("https://asrlab-production.up.railway.app/lims/master/get-nodalcenter", { headers: { Authorization: `Bearer ${authToken}` } }),
        ...
        setSubDepartments(subDeptRes.data.filter((s) => s.isActive));
        setRoleTypes(roleRes.data.filter((r) => r.isactive));
        setHospitalTypes(hospRes.data.filter((h) => h.isActive));
        setSpecimens(specRes.data.filter((s) => s.isactive));
        setNodalCenters(nodalRes.data);
        setInstruments(instrRes.data);
      } catch (error) {
        setFetchError("❌ Failed to fetch master data.");
      }
    };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const authToken = localStorage.getItem("authToken");
      await axios.post(
        "https://asrlab-production.up.railway.app/lims/master/add-technician",
        payload,
          ...
          <div className="p-6">
            ...
            <div className="flex justify-end mt-8">
              <button
                ...
                className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-2 rounded-lg font-semibold shadow-md disabled:opacity-50"
              >
