...
const UpdateInvestigation = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // Step 1️⃣: Update main investigation
      await updateInvestigation(investigation.id, payload);
      console.log("Main investigation updated successfully ✅");
  
      const token = localStorage.getItem("authToken");
  
      // Step 2️⃣: Update each result
      const updateResults = results.map(async (res) => {
        const resultPayload = {
          resultname: res.resultname,
          unit: res.unit,
          valueType: res.valueType,
          ...
          order: res.order,
          roundOff: res.roundOff,
          showTrends: res.showTrends,
          defaultValue: res.defaultValue,
        };
  
        const apiUrl = `https://asrlabs.asrhospitalindia.in/lims/master/update/${investigation.id}/results/${res.id}`;
        ...
        if (!response.ok) {
          console.error(`❌ Failed to update result ${res.id}`);
        } else {
          console.log(`✅ Result ${res.id} updated successfully`);
        }
      });
      ...
      toast.error("❌ Failed to update investigation, results or normal values");
      console.error(err);
    } finally {
