...
import { FaSortUp, FaSortDown, FaSort, FaPrint } from "react-icons/fa";
...
const PrintSectionDataTable = ({
  items = [],
  columns = [],
  itemsPerPage = 5,
  defaultSorter = { column: "status", state: "asc" },
  showDetailsButtons = true,
  onPrint, // 👈 new prop for print invoice action
  ...
  const baseColumns = useMemo(
      ...
      ...transformedColumns,
      {
        ...
        cell: ({ row }) => (
          <div className="flex justify-center">
            <button
              className="p-2 bg-blue-100 rounded-full hover:bg-blue-600"
              onClick={() => onPrint && onPrint(row.original)}
              title="Print Invoice"
            >
              <FaPrint className="text-blue-700 hover:text-white" />
            </button>
          </div>
        ),
      },
    ...
    [transformedColumns, expandedRows, onPrint, showDetailsButtons]
  );
        ...
        <tbody className="divide-y divide-gray-200">
          {table.getRowModel().rows.map((row) => (
            <React.Fragment key={row.original.id}>
              <tr
                className={`cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-y-[1.02] hover:bg-blue-50 ${
...
export default PrintSectionDataTable;
